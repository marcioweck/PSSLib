...
tFitness CEC2013::evaluate(const std::vector<double> &x)
{
	assert(x.size() == dimensions_[nfunc_-1]);
	return evaluate(&x[0]);
}

// DEPRECATED
// TODO: rename the function adequately and make it returns the
// seeds in the population corresponding to global optima. The
// current algorithm is inefficient for the first intented purpose
// of this function, but the logic is still interesting.
int CEC2013::how_many_goptima(const std::vector< std::vector<double> > &pop,
		std::vector< std::vector<double> > &seeds,
		const double &accuracy)
{
	std::vector<std::vector<double> > sorted_pop;
	/* Evaluate pop */
	vector<tFitness> fits;//(pop.size());
	for (std::vector< std::vector<double> >::const_iterator it = pop.begin(); it != pop.end(); ++it) {
		fits.push_back( evaluate(*it) );
	}
	...
	get_seeds(sorted_pop, seeds, get_rho());
	std::cout << seeds.size() << std::endl;
	std::cout << get_rho() << std::endl;
	/* Based on the accuracy: check which seeds are global optimizers */
	int count(0);
	for (std::vector< std::vector<double> >::const_iterator sit = seeds.begin(); sit != seeds.end(); ++sit) {
		/* evaluate seed */
		tFitness seed_fitness = evaluate(*sit);
		/* |F_seed - F_goptimum| <= accuracy */
		if ( fabs(seed_fitness - get_fitness_goptima()) <= accuracy) {
			++count;
		}
		/* save time */
		if (count == get_no_goptima()) break;
	}
	return count;
}

// Count effeciently the number of global optima present
// in the population given a a vector of individuals, a
// vector of their respective fitness and an accuracy level.
// This method replaces how_many_goptima. It first determine
// which individuals are optima, then compare these optima
// to determine the number of unique optima.
int CEC2013::count_goptima(const std::vector<std::vector<double> > &pop,
						   const std::vector<double> &fit,
						   double accuracy)
{
	tFitness optfitness = get_fitness_goptima();
	std::vector<std::vector<double> > optimums;
	for (size_t i = 0; i < pop.size(); ++i) {
		if(fabs(fit[i] - optfitness) <= accuracy) {
			optimums.push_back(pop[i]);
		}
	}

	double rho = get_rho();
	std::vector<int> seeds;
	for(size_t i = 0; i < optimums.size(); ++i){
		bool found = false;
		for(size_t j = 0; j < seeds.size(); ++j){
			if(get_eudist(optimums[i], optimums[seeds[j]]) <= rho){
				found = true;
				break;
			}
		}

		if(!found) seeds.push_back(i);
		//EXTREMELY BOGUS - should check again this could
		//mean that we actually found more global optima than
		//there is because of a bad choice of accuracy and rho.
		if (seeds.size() == get_no_goptima()) break;
	}
	return seeds.size();
}
		...
		const double &radius)
{
	...
	for (std::vector< std::vector<double> >::const_iterator it = pop.begin(); it != pop.end(); ++it) {
		...
		for (std::vector< std::vector<double> >::iterator sit = seeds.begin(); sit != seeds.end(); ++sit) {
			...
			// Stop the loop if the Euclidean distance is less than the radius
			if (dist <= radius) {
				found = true;
				break;
			}
